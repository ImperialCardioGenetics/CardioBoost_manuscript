...
tune_control_glmnet <- makeTuneControlRandom(maxit = 200)

cvglmnet<-makeLearner("classif.cvglmnet",predict.type="prob")
cvglmnet_grid<-makeParamSet(makeNumericParam("alpha",lower=0,upper=1))

res<-tuneParams(cvglmnet,task=task,resampling = inter,par.set =cvglmnet_grid,control = tune_control_glmnet,measures=list(self_measure))

ada_tuned <- makeLearner("classif.cvglmnet",predict.type = "prob",alpha=res$x$alpha)
...
save(bmr_cvglmnet_full,file="../../../data/cardiomyopathy/ml/bmr_cvglmnet.RData")
