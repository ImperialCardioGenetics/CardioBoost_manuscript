...
require(ggplot2)
getwd()
load("../../data/raw/cm_cohort.RData")
load("../../data/raw/cm_hvol.RData")
...
overlap_key<-intersect(cohort[which(cohort$pathogenic==1),]$key,hvol_cm$key)
cohort<-subset(cohort,!is.element(key,overlap_key))
hvol_cm<-subset(hvol_cm,!is.element(key,overlap_key))

train<-rbind(cohort,hvol_cm)  
...
save(impute_mean,scale_mean,scale_std,mean_imputation,z_normalization,train_imputed,file="../../data/processed/preprocess.RData")
save(train_scaled,file="../../data/processed/train_scaled.RData")
