---
title: "Evaluation on hold-out set on Arrhythmia variants"
...
TPR<-c(measureTPR(test_prediction$pathogenic,ifelse(test_prediction$pathogenicity>=0.90,1,0),1),
measureTPR(test_prediction$pathogenic,ifelse(test_prediction$MCAP>=0.90,1,0),1),
measureTPR(test_prediction$pathogenic,ifelse(test_prediction$REVEL>=0.90,1,0),1))


PPV<-c(measurePPV(test_prediction$pathogenic,ifelse(test_prediction$pathogenicity>0.9,1,0),1),
measurePPV(test_prediction$pathogenic,ifelse(test_prediction$MCAP>0.9,1,0),1),
measurePPV(test_prediction$pathogenic,ifelse(test_prediction$REVEL>0.9,1,0),1))



FPR<-c(measureFPR(test_prediction$pathogenic,ifelse(test_prediction$pathogenicity>0.9,1,0),0,1),
measureFPR(test_prediction$pathogenic,ifelse(test_prediction$MCAP>0.9,1,0),0,1),
measureFPR(test_prediction$pathogenic,ifelse(test_prediction$REVEL>0.9,1,0),0,1))


TNR<-c(measureTNR(test_prediction$pathogenic,ifelse(test_prediction$pathogenicity>=0.10,1,0),0),
measureTNR(test_prediction$pathogenic,ifelse(test_prediction$MCAP>=0.10,1,0),0),
measureTNR(test_prediction$pathogenic,ifelse(test_prediction$REVEL>=0.10,1,0),0))

NPV<-c(measureNPV(test_prediction$pathogenic,ifelse(test_prediction$pathogenicity>=0.10,1,0),0),
measureNPV(test_prediction$pathogenic,ifelse(test_prediction$MCAP>=0.10,1,0),0),
measureNPV(test_prediction$pathogenic,ifelse(test_prediction$REVEL>=0.10,1,0),0))


FNR<-c(measureFNR(test_prediction$pathogenic,ifelse(test_prediction$pathogenicity>=0.10,1,0),0,1),
measureFNR(test_prediction$pathogenic,ifelse(test_prediction$MCAP>=0.10,1,0),0,1),
measureFNR(test_prediction$pathogenic,ifelse(test_prediction$REVEL>=0.10,1,0),0,1))

pr_clin<-data.frame(TPR,PPV)

clin<-c((sum(test_prediction$pathogenicity<=0.1)+sum(test_prediction$pathogenicity>=0.9))/nrow(test),(sum(test_prediction$MCAP<=0.1)+sum(test_prediction$MCAP>=0.9))/nrow(test),(sum(test_prediction$REVEL<=0.1)+sum(test_prediction$REVEL>=0.9))/nrow(test))

Acc_clin<-c((sum(test_prediction$pathogenicity[test_prediction$pathogenic==0]<=0.1)+sum(test_prediction$pathogenicity[test_prediction$pathogenic==1]>=0.9))/(sum(test_prediction$pathogenicity<=0.1)+sum(test_prediction$pathogenicity>=0.9)),(sum(test_prediction$MCAP[test_prediction$pathogenic==0]<=0.1)+sum(test_prediction$MCAP[test_prediction$pathogenic==1]>=0.9))/(sum(test_prediction$MCAP<=0.1)+sum(test_prediction$MCAP>=0.9)),(sum(test_prediction$REVEL[test_prediction$pathogenic==0]<=0.1)+sum(test_prediction$REVEL[test_prediction$pathogenic==1]>=0.9))/(sum(test_prediction$REVEL<=0.1)+sum(test_prediction$REVEL>=0.9)))

Acc<-c((sum(test_prediction$pathogenicity[test_prediction$pathogenic==0]<=0.1)+sum(test_prediction$pathogenicity[test_prediction$pathogenic==1]>=0.9))/nrow(test),(sum(test_prediction$MCAP[test_prediction$pathogenic==0]<=0.1)+sum(test_prediction$MCAP[test_prediction$pathogenic==1]>=0.9))/nrow(test),(sum(test_prediction$REVEL[test_prediction$pathogenic==0]<=0.1)+sum(test_prediction$REVEL[test_prediction$pathogenic==1]>=0.9))/nrow(test))

In_acc<-c((sum(test_prediction$pathogenicity[test_prediction$pathogenic==0]>=0.9)+sum(test_prediction$pathogenicity[test_prediction$pathogenic==1]<=0.1))/nrow(test),(sum(test_prediction$MCAP[test_prediction$pathogenic==0]>=0.9)+sum(test_prediction$MCAP[test_prediction$pathogenic==1]<=0.1))/nrow(test),(sum(test_prediction$REVEL[test_prediction$pathogenic==0]>=0.9)+sum(test_prediction$REVEL[test_prediction$pathogenic==1]<=0.1))/nrow(test))


Non_clin<-c(nrow(subset(test_prediction,pathogenicity>0.1 & pathogenicity<0.9))/nrow(test),nrow(subset(test_prediction,MCAP>0.1 & MCAP<0.9))/nrow(test),nrow(subset(test_prediction,REVEL>0.1 &  REVEL<0.9))/nrow(test))
...
ppv.f<-function(data){
  measurePPV(data$class,ifelse(data$prob_pathogenic>=0.90,1,0),1)
}
permutation(p_ada,p_revel,fun=ppv.f,n=10000)

###FPR

fpr.f<-function(data){
  measureFPR(data$class,ifelse(data$prob_pathogenic>=0.90,1,0),0,1)
}
...
permutation(p_ada,p_revel,fun=fpr.f,n=10000)


###TNR
tnr.f<-function(data){
  measureTNR(data$class,ifelse(data$prob_pathogenic<=0.10,0,1),0)
}
...
npv.f<-function(data){
  ...
  measureNPV(data$class,ifelse(data$prob_pathogenic<=0.10,0,1),0)
...
fnr.f<-function(data){
  ...
  measureFNR(data$class,ifelse(data$prob_pathogenic<=0.10,0,1),0,1)
  
}
permutation(p_ada,p_mcap,fun=fnr.f,n=10000)


###Clinicial Actionable variants
clin.f<-function(data){
  (sum(data$prob_pathogenic<=0.1)+sum(data$prob_pathogenic>=0.9))/nrow(data)
}
...
clin_acc.f<-function(data){
  (sum(data$prob_pathogenic[data$class==0]<=0.1)+sum(data$prob_pathogenic[data$class==1]>=0.9))/(sum(data$prob_pathogenic<=0.1)+sum(data$prob_pathogenic>=0.9))
}
...
###Accuracy
acc.f<-function(data){
  (sum(data$prob_pathogenic[data$class==0]<=0.1)+sum(data$prob_pathogenic[data$class==1]>=0.9))/nrow(data)
...
non_clin.f<-function(data){
  nrow(subset(data,prob_pathogenic>0.1 & prob_pathogenic<0.9))/nrow(data)
}
...
### Figure 2: (D)(E) Precision-Recall Curves and ROC Curves
...
#significance test
roc.test(cardioboost_roc,revel_roc,method="delong")
...
#significance test
permutation(p_ada,p_revel,fun=pr.f,n=10000)
