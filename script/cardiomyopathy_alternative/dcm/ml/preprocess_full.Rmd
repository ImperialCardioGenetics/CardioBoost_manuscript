...
#Build a model that can predict variants only causing DCM but not HCM
#Variants only causing HCM would be considered as benign to DCM

overlap_key<-intersect(dcm_cohort[which(dcm_cohort$pathogenic==1),]$key,hvol_cm$key)
dcm_cohort<-subset(dcm_cohort,!is.element(key,overlap_key))
hvol_cm<-subset(hvol_cm,!is.element(key,overlap_key))

dcm_cohort<-subset(dcm_cohort,select=-c(disease))

hvol_cm<-subset(hvol_cm,!is.element(key,hcm_cohort[which(hcm_cohort$pathogenic==1),]$key))

hcm_cohort<-subset(hcm_cohort,pathogenic==1)
hcm_cohort<-subset(hcm_cohort,!is.element(key,dcm_cohort[which(dcm_cohort$pathogenic==1),]$key))
hcm_cohort$pathogenic=0
hcm_cohort<-subset(hcm_cohort,select=-c(disease))

train<-rbind(dcm_cohort,hvol_cm,hcm_cohort)  
...
save(train,file="../../../../data/cardiomyopathy_alternative/dcm/dcm_raw_train.RData")

load("../../../../data/cardiomyopathy_alternative/dcm/dcm_raw_train.RData")
...
save(train,file="../../../../data/cardiomyopathy_alternative/dcm/dcm_train.RData")
save(test,file="../../../../data/cardiomyopathy_alternative/dcm/dcm_holdout_test.RData")
...
save(impute_features,impute_mean,scale_mean,scale_std,mean_imputation,z_normalization,feature_column,disease_gene,train_imputed,file="../../../../data/cardiomyopathy_alternative/dcm/preprocess.RData")
save(train_scaled,file="../../../../data/cardiomyopathy_alternative/dcm/train_scaled.RData")
