...
factor_column<-c("SIFT_score","Polyphen2_HDIV_score","Polyphen2_HVAR_score","LRT_score","MutationAssessor_score","FATHMM_score","PROVEAN_score","MCAP","PARASCOREzSCORE")

arm_clinvar[,factor_column]<-sapply(factor_column,function(x){numeric_column(arm_clinvar,x)})

character_column<-c("SIFT_score","LRT_score","FATHMM_score","PROVEAN_score","MCAP","PARASCOREzSCORE")
arm_hvol[,character_column]<-sapply(character_column,function(x){numeric_column(arm_hvol,x)})
...
save(train,file="../../../data/arrhythmia/arm_raw_training.RData")

load("../../../data/arrhythmia/arm_raw_training.RData")

train<-subset(train,(gnomAD_AF<=0.001 | is.na(gnomAD_AF) | gnomAD_AF!=0))

exclude_gene<-setdiff(unique(train[which(train$pathogenic==0),]$gene),unique(train[which(train$pathogenic==1),]$gene))
#excluding the genes that have week evidences
...
test<-rbind(sample_n(train[which(train$pathogenic==1),],length(which(train$pathogenic==1))*1/3),sample_n(train[which(train$pathogenic==0),],length(which(train$pathogenic==0))*1/3))
