...
require(dplyr)
getwd()
load("../../../data/myh7_hcm/cm_hcm_cohort.RData")
load("../../../data/myh7_hcm/hvol_cm.RData")
load("../../../data/myh7_hcm/cm_dcm_cohort.RData")
...
myh7_cohort<-subset(hcm_cohort,gene=="MYH7")
overlap_key<-intersect(myh7_cohort[which(myh7_cohort$pathogenic==1),]$key,hvol_cm$key)
myh7_cohort<-subset(myh7_cohort,!is.element(key,overlap_key))
hvol_cm<-subset(hvol_cm,!is.element(key,overlap_key))
myh7_dcm<-subset(dcm_cohort,gene=="MYH7" & pathogenic==1)
myh7_dcm<-subset(dcm_cohort,!is.element(key,myh7_cohort[which(myh7_cohort$pathogenic==1),]$key))
myh7_dcm$pathogenic=0

train<-rbind(myh7_cohort,hvol_cm,myh7_dcm)  
...
train_pathogenic<-subset(train,pathogenic==1)
train_benign<-subset(train,pathogenic==0)
train<-rbind(train_pathogenic,train_benign)
train<-subset(train,gene=="MYH7")


test_pathogenic<-subset(test,pathogenic==1)
test_benign<-subset(test,pathogenic==0)
test<-rbind(test_pathogenic,test_benign)
test<-subset(test,gene=="MYH7")

save(train,file="../../../data/myh7_hcm/myh7_train.RData")
save(test,file="../../../data/myh7_hcm/myh7_holdout_test.RData")
...
save(impute_features,impute_mean,scale_mean,scale_std,mean_imputation,z_normalization,feature_column,disease_gene,train_imputed,file="../../../data/myh7_hcm/preprocess.RData")
save(train_scaled,file="../../../data/myh7_hcm/train_scaled.RData")
