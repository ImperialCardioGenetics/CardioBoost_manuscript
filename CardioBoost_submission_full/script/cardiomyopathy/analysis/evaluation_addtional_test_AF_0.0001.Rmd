...
load("../../../data/cardiomyopathy/cm_additional_test_pathogenic_raw.RData")
cm_patho_test$key<-paste(cm_patho_test$CHROM,cm_patho_test$POS,cm_patho_test$REF,cm_patho_test$ALT,sep="_")
cm_patho_test$gnomAD_AF<-cm_prediction[match(cm_patho_test$key,cm_prediction$key),]$gnomAD_AF
...
cm_patho_test<-subset(cm_patho_test,gnomAD_AF<0.0001 | is.na(gnomAD_AF))
...
### Benign test set (gnomAD_subset): true negative rate at P<0.1
```{r}
cm_gnomad<-subset(cm_prediction,is.na(AF_Adj)==TRUE)

cm_gnomad_rare_noexac<-subset(cm_gnomad,gnomAD_AF<0.0001|is.na(gnomAD_AF))
...
### Benign test set (gnomAD_subset): true negative rate at P<0.05
```{r}
